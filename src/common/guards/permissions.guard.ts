...
export class PermissionsGuard implements CanActivate {
	...
	async checkUserWorkspacePermission(workspaceId: string, userId: string, ability: string) {
	...
	async canActivate(ctx: ExecutionContext): Promise<boolean> {
		...
		const workspaceId: string = (request.query["workspaceId"] as string) || (request.headers["workspaceId"] as string)
		...
		const isAllowed = await this.checkUserWorkspacePermission(workspaceId, user.id, ability)
		...
		// access denied error
		throw new ForbiddenException(`User is not allowed to ${ability} on workspace ${workspaceId}`)
	}
}
