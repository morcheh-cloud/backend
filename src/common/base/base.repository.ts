...
> extends Repository<Entity> {
  ...
  override async findOne(
    optionsOrConditions: CustomFindOneOptions<Entity>
  ): Promise<Entity | null> {
  ...
  override async find(
    ...
    if (
      typeof optionsOrConditions?.page === "number" &&
      typeof optionsOrConditions.limit === "number"
    ) {
      pagination = Paginate(
        optionsOrConditions.page,
        optionsOrConditions.limit
      );
    }
  ...
  async createAndSave(
    ...
    const clearedData: Record<string, any> = omitBy(
      data as any,
      (key: string) => isUndefined(key)
    );
  ...
  private customCreateQueryBuilder(
    alias?: string,
