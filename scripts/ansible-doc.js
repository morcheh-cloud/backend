...
async function main() {
	...
	for (const moduleName of moduleNames) {
		const { stdout: ansibleDocRaw } = await execAsync(`ansible-doc -j ${moduleName}`)
		...
		if (exist) {
			...
			await client.query(
				...
				[description, isOfficial ? "official" : "community", schema, moduleName],
			)
		} else {
			// insert
			await client.query(
		...
		console.info(`Processed module: ${moduleName.padEnd(60)}`, `(${counter++}/${size})`)
	}
}
